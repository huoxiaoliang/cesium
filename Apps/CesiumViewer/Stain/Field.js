...
class Field {
  constructor(params) {
    ...
    if (Number(cols) !== this.cols || Number(rows) !== this.rows) {
      // eslint-disable-next-line no-console
  ...
  uvMinMax(u, v) {
    if (!this.minU) this.minU = u;
    this.minU = Math.min(this.minU, u);
    if (!this.minV) this.minV = v;
    this.minV = Math.min(this.minV, v);

    if (!this.maxU) this.maxU = u;
    this.maxU = Math.max(this.maxU, u);
    if (!this.maxV) this.maxV = v;
  ...
  /**
   * Bilinear interpolation for Vector 双线性插值
