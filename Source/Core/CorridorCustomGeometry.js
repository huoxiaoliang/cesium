...
import CorridorCustomGeometryLibrary from "./CorridorCustomGeometryLibrary.js";
...
function addNormals(attr, normal, left, front, back, vertexFormat) {
  ...
  if (vertexFormat.normal) {
    CorridorCustomGeometryLibrary.addAttribute(normals, normal, front, back);
  }
  if (vertexFormat.tangent) {
    CorridorCustomGeometryLibrary.addAttribute(tangents, forward, front, back);
  }
  if (vertexFormat.bitangent) {
    CorridorCustomGeometryLibrary.addAttribute(bitangents, left, front, back);
  }
}
...
function combine(computedPositions, vertexFormat, ellipsoid) {
  ...
  if (addEndPositions) {
    ...
    for (i = 0; i < halfLength; i++) {
      ...
      CorridorCustomGeometryLibrary.addAttribute(
        finalPositions,
        rightPos,
        front
      );
  ...
  if (vertexFormat.normal || vertexFormat.bitangent || vertexFormat.tangent) {
    ...
    for (i = 0; i < threeSize; i += 3) {
      ...
      if (vertexFormat.normal) {
        ...
        CorridorCustomGeometryLibrary.addAttribute(normals, normal, attrIndex);
        CorridorCustomGeometryLibrary.addAttribute(
          normals,
          normal,
          attrIndex + 3
...
function computePositionsExtruded(params, vertexFormat) {
  ...
  var computedPositions = CorridorCustomGeometryLibrary.computePositions(
    params
  );
 ...
 *
 * @see CorridorCustomGeometry.createGeometry
 * @see Packable
 *
 * @demo {@link https://sandcastle.cesium.com/index.html?src=Corridor.html|Cesium Sandcastle Corridor Demo}
 *
 * @example
 * var corridor = new Cesium.CorridorCustomGeometry({
 *   vertexFormat : Cesium.VertexFormat.POSITION_ONLY,
 *   positions : Cesium.Cartesian3.fromDegreesArray([-72.0, 40.0, -70.0, 35.0]),
 *   width : 100000
 * });
 */
function CorridorCustomGeometry(options) {
  ...
  this._workerName = "createCorridorCustomGeometry";
 ...
 *
 * @param {CorridorCustomGeometry} value The value to pack.
 * @param {Number[]} array The array to pack into.
 * @param {Number} [startingIndex=0] The index into the array at which to start packing the elements.
 *
 * @returns {Number[]} The array that was packed into
 */
CorridorCustomGeometry.pack = function (value, array, startingIndex) {
  //>>includeStart('debug', pragmas.debug);
 ...
 * @param {CorridorCustomGeometry} [result] The object into which to store the result.
 * @returns {CorridorCustomGeometry} The modified result parameter or a new CorridorCustomGeometry instance if one was not provided.
 */
CorridorCustomGeometry.unpack = function (array, startingIndex, result) {
  //>>includeStart('debug', pragmas.debug);
  Check.defined("array", array);
  //>>includeEnd('debug');
  ...
  if (!defined(result)) {
    ...
    return new CorridorCustomGeometry(scratchOptions);
  }
 ...
 */
CorridorCustomGeometry.computeRectangle = function (options, result) {
  options = defaultValue(options, defaultValue.EMPTY_OBJECT);
  var positions = options.positions;
  var width = options.width;
 ...
 *
 * @param {CorridorCustomGeometry} CorridorCustomGeometry A description of the corridor.
 * @returns {Geometry|undefined} The computed vertices and indices.
 */
CorridorCustomGeometry.createGeometry = function (CorridorCustomGeometry) {
  var positions = CorridorCustomGeometry._positions;
  var width = CorridorCustomGeometry._width;
  var ellipsoid = CorridorCustomGeometry._ellipsoid;
  ...
  var height = CorridorCustomGeometry._height;
  var extrudedHeight = CorridorCustomGeometry._extrudedHeight;
  ...
  var vertexFormat = CorridorCustomGeometry._vertexFormat;
  var params = {
    ellipsoid: ellipsoid,
    positions: positions,
    width: width,
    cornerType: CorridorCustomGeometry._cornerType,
    granularity: CorridorCustomGeometry._granularity,
    saveAttributes: true,
  };
  var attr;
  if (extrude) {
    ...
    params.shadowVolume = CorridorCustomGeometry._shadowVolume;
    params.offsetAttribute = CorridorCustomGeometry._offsetAttribute;
    ...
    if (defined(CorridorCustomGeometry._offsetAttribute)) {
      var applyOffsetValue =
        CorridorCustomGeometry._offsetAttribute === GeometryOffsetAttribute.NONE
          ? 0
          : 1;
  ...
  return new Geometry({
    ...
    offsetAttribute: CorridorCustomGeometry._offsetAttribute,
  });
};
 ...
 * @private
 */
CorridorCustomGeometry.createShadowVolume = function (
  CorridorCustomGeometry,
  minHeightFunc,
  maxHeightFunc
) {
  var granularity = CorridorCustomGeometry._granularity;
  var ellipsoid = CorridorCustomGeometry._ellipsoid;

  var minHeight = minHeightFunc(granularity, ellipsoid);
  var maxHeight = maxHeightFunc(granularity, ellipsoid);

  return new CorridorCustomGeometry({
    positions: CorridorCustomGeometry._positions,
    width: CorridorCustomGeometry._width,
    cornerType: CorridorCustomGeometry._cornerType,
    ellipsoid: ellipsoid,
...
Object.defineProperties(CorridorCustomGeometry.prototype, {
  /**
   * @private
   */
  rectangle: {
...
export default CorridorCustomGeometry;
