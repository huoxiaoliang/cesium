...
uniform highp float u_inverseTileWidth;
uniform highp vec2 u_cartographicTileRectangle;
uniform highp sampler2D u_flat_AreaTexture;
const float invalidValue = 8654238.5581; // 设置无效值
vec4 getRegions(int x, int y) {
    float u = (float(x) + 0.5) / float(u_flat_AreaHeight);
    float v = (float(y) + 0.5) / float(u_flat_AreaWidth);
